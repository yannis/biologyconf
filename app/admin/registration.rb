...
ActiveAdmin.register Registration do
  ...
  form do |f|
    ...
    f.inputs "Abstract" do
      ...
      f.input :poster_number, as: :select, collection: (((1..60).to_a-Registration.select(:poster_number).map(&:poster_number))<< f.object.poster_number).sort.map{|i| [i,i]}, include_blank: true
      f.input :title
      f.input :authors
      f.input :body
    end
